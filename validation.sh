...
if grep -qiE '^[[:space:]]*(COPY|ADD)[[:space:]]' "$DOCKERFILE"; then
    echo "ERROR: Dockerfile must not use COPY or ADD (runtime mounts only): $DOCKERFILE" >&2
    exit 1
fi
...
docker exec -u root -w /eval_assets "$CONTAINER_ID" \
    /bin/bash -c 'python3 /eval_assets/parse_results stdout.txt stderr.txt before.json || echo "{\"tests\": []}" > before.json; test -f before.json || echo "{\"tests\": []}" > before.json'
...
docker exec -u root -w /eval_assets "$CONTAINER_ID" \
    /bin/bash -c 'python3 /eval_assets/parse_results stdout.txt stderr.txt after.json || echo "{\"tests\": []}" > after.json; test -f after.json || echo "{\"tests\": []}" > after.json'
